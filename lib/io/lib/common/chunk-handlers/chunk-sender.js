...
ChunkSender.prototype.cancel = function() {
	this.paused = true;
	this.send(Enum.Pipe.Command.CANCEL_TRANSFER, false, this.transferParam);
};

ChunkSender.prototype.executeCancel = function() {
	var self = this;
	this.chunkReader.cancel(function(error) {
		if (!error) {
			self.send(Enum.Pipe.Command.CANCEL_TRANSFER, true, self.transferParam);
			if (self.callback) self.callback(null, self.result(true));

		} else {
			self.error('ChunkReader:' + error.message);
		}
	});
};

ChunkSender.prototype.acknowledgeCancel = function(clientResponse) {
	this.chunkReader.acknowledgeCancel(clientResponse);
};

ChunkSender.prototype.endCancel = function() {
