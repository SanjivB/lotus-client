...
ChunkHandlers.prototype.autoPause = function() {
	var self = this;
	for (var key in self.items) {
		var chunkHandler = self.items[key];
		chunkHandler.autoPause();
	}
}

ChunkHandlers.prototype.autoResume = function() {
	var self = this;
	for (var key in self.items) {
		var chunkHandler = self.items[key];
		chunkHandler.autoResume();
	}
}
...
ChunkHandlers.prototype.executeCancel = function(id) {
	var item = this.item(id);
	if (item) {
		item.executeCancel();
	}
...
ChunkHandlers.prototype.acknowledgeCancel = function(id) {
	var item = this.item(id);
	if (item) {
		item.acknowledgeCancel();
	}
};

ChunkHandlers.prototype.endCancel = function(id) {
	var item = this.item(id);
	if (item) {
		item.endCancel();
	}
